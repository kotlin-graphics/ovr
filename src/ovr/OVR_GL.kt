 ...
 *  \param[out] out_TextureSwapChain Returns the created ovrTextureSwapChain, which will be valid upon a successful return value, else it will be NULL. This
 *             texture swap chain must be eventually destroyed via ovr_DestroyTextureSwapChain before destroying the session with ovr_Destroy.
 *
 ...
 *
 *  \note The \a format provided should be thought of as the format the distortion compositor will use when reading the contents of the texture. To that end, it is
 *  highly recommended that the application requests texture swap chain formats that are in sRGB-space (e.g. OVR_FORMAT_R8G8B8A8_UNORM_SRGB) as the distortion
 *  compositor does sRGB-correct rendering. Furthermore, the app should then make sure "glEnable(GL_FRAMEBUFFER_SRGB);" is called before rendering into these
 *  textures. Even though it is not recommended, if the application would like to treat the texture as a linear format and do linear-to-gamma conversion in GLSL,
 *  then the application can avoid calling "glEnable(GL_FRAMEBUFFER_SRGB);", but should still pass in an sRGB variant for the \a format. Failure to do so will
 *  cause the distortion compositor to apply incorrect gamma conversions leading to gamma-curve artifacts.
 *
...
external fun ovr_CreateTextureSwapChainGL(session: OvrSession, desc: OvrTextureSwapChainDesc.ByReference, out_TextureSwapChain: OvrTextureSwapChain_ByReference)
        : OvrResult
 ...
 *  \param[in ]  index Specifies the index within the chain to retrieve. Must be between 0 and length (see ovr_GetTextureSwapChainLength) or may pass -1 to get the
 *              buffer at the CurrentIndex location. (Saving a call to GetTextureSwapChainCurrentIndex)
 *  \param[out ] out_TexId Returns the GL texture object name associated with the specific index requested
 *
 *  \return Returns an ovrResult indicating success or failure. In the case of failure, use ovr_GetLastErrorInfo to get more information.    */
external fun ovr_GetTextureSwapChainBufferGL(session: OvrSession, chain: OvrTextureSwapChain, index: Int, out_TexId: IntByReference): OvrResult
...
external fun ovr_CreateMirrorTextureGL(session: OvrSession, desc: OvrMirrorTextureDesc.ByReference, out_MirrorTexture: ovrMirrorTexture_ByReference): OvrResult
 ...
 *
 *  \return Returns an ovrResult indicating success or failure. In the case of failure, use ovr_GetLastErrorInfo to get more information.    */
external fun ovr_GetMirrorTextureBufferGL(session: OvrSession, mirrorTexture: ovrMirrorTexture, out_TexId: IntByReference): OvrResult
